    ...
    for (uint32_t i = 0; i < agg_exprs_.size(); i++) {
      switch (agg_types_[i]) {
        ...
        case AggregationType::MinAggregate:
          ...
          //2. 比现有的小
          if(!input.aggregates_[i].IsNull() &&
            (result->aggregates_[i].IsNull() || input.aggregates_[i].CompareLessThan(result->aggregates_[i]) == CmpBool::CmpTrue)){
        ...
        case AggregationType::MaxAggregate:
          ...
          //2. 比现有的大
          if(!input.aggregates_[i].IsNull() &&
            (result->aggregates_[i].IsNull() || input.aggregates_[i].CompareGreaterThan(result->aggregates_[i]) == CmpBool::CmpTrue)){
              result->aggregates_[i] = input.aggregates_[i];
            }
          break;
      }
